...
#pragma warning(pop)
	...
	void BootEncryption::WriteBootSectorConfig (const byte newConfig[])
	{
		...
		if (memcmp (mbr, mbrVerificationBuf, sizeof (mbr)) != 0)
			//TODO: more info on exception "ERROR_MBR_PROTECTED"
...
#	error TC_BOOT_CFG_FLAG_AREA_SIZE != 1; revise GetHiddenOSCreationPhase() and SetHiddenOSCreationPhase()
	...
	BOOL BootEncryption::IsHiddenOSRunning (void)
	{
		...
		if (!statusCached)
		{
			try
			{
				hiddenOSRunning = IsHiddenSystemRunning();
			}
			catch (...)
			{
...
#	error PRAND_DISK_WIPE_PASSES > RNG_POOL_SIZE
	...
	void BootEncryption::RegisterFilter (bool registerFilter, FilterType filterType, const GUID *deviceClassGuid)
	{
		...
		else
		{
			...
			//TODO: handle dialog windows
			throw_sys_if (!SetupInstallFromInfSection (NULL, hInf, "truecrypt", SPINST_REGISTRY, regKey, NULL, 0, NULL, NULL, NULL, NULL));
		}
	}
	...
	void BootEncryption::CheckRequirements ()
	{
		if (nCurrentOS == WIN_2000)
			//TODO: "SYS_ENCRYPTION_UNSUPPORTED_ON_CURRENT_OS"
			throw ErrorException ();

		if (CurrentOSMajor == 6 && CurrentOSMinor == 0 && CurrentOSServicePack < 1)
			//TODO: "SYS_ENCRYPTION_UNSUPPORTED_ON_VISTA_SP0"
			throw ErrorException ();

		if (IsNonInstallMode())
			//TODO: "FEATURE_REQUIRES_INSTALLATION"
			throw ErrorException ();

		SystemDriveConfiguration config = GetSystemDriveConfiguration ();

		if (config.SystemPartition.IsGPT)
			//TODO: "GPT_BOOT_DRIVE_UNSUPPORTED"
			throw ErrorException ();

		if (SystemDriveIsDynamic())
			//TODO: "SYSENC_UNSUPPORTED_FOR_DYNAMIC_DISK"
			throw ErrorException ();

		if (config.InitialUnallocatedSpace < TC_BOOT_LOADER_AREA_SIZE)
			//TODO: "NO_SPACE_FOR_BOOT_LOADER"
			throw ErrorException ();

		DISK_GEOMETRY geometry = GetDriveGeometry (config.DriveNumber);

		if (geometry.BytesPerSector != TC_SECTOR_SIZE_BIOS)
			//TODO: "SYSENC_UNSUPPORTED_SECTOR_SIZE_BIOS"
			throw ErrorException ();
	...
	void BootEncryption::InitialSecurityChecksForHiddenOS ()
	{
		...
		if (!pagingFilesOk)
		{
			...
			//TODO: "PAGING_FILE_NOT_ON_SYS_PARTITION" "LEAKS_OUTSIDE_SYSPART_UNIVERSAL_EXPLANATION"
			throw ErrorException ();
		}
		...
		{
			//TODO: "USER_PROFILE_NOT_ON_SYS_PARTITION" "LEAKS_OUTSIDE_SYSPART_UNIVERSAL_EXPLANATION"
			throw ErrorException ();
		}
		...
		{
			//TODO: "TEMP_NOT_ON_SYS_PARTITION" "LEAKS_OUTSIDE_SYSPART_UNIVERSAL_EXPLANATION"
			throw ErrorException ();
		}
	}
	...
	int BootEncryption::ChangePassword (Password *oldPassword, Password *newPassword, int pkcs5)
	{
		...
		{
			//TODO: all the e.Show () need to be checked
			e.Show ();
			result = ERR_OS_ERROR;
		}
	...
	void BootEncryption::Deinstall (bool displayWaitDialog)
	{
		...
		catch (Exception &e)
		{
			//TODO:
			e.Show ();
			//TODO: "SYS_LOADER_RESTORE_FAILED"
			throw ErrorException ();
		}
	}
